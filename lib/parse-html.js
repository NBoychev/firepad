    ...
    switch (node.nodeType) {
      ...
      case Node.ELEMENT_NODE:
        ...
        switch (node.nodeName.toLowerCase()) {
          ...
          case 'h2':
          case 'h3':
            var lineFormatting = Firepad.LineFormatting({ heading: node.nodeName.toLowerCase() });
            state = state.withLineFormatting(lineFormatting);
            output.newlineIfNonEmpty(state);
            parseChildren(node, state.withLineFormatting(lineFormatting), output);
            output.newlineIfNonEmpty(state);
            break;
          case 'center':
